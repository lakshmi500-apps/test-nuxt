...
<div class="border-b border-gray-200 bg-white px-4 py-5 sm:px-6">
  ...
  <button class="resize-y rounded-md" action="#">
    ...
    @addContact="openAddContactsModal"
    @deleteContact="deleteModal"
    @editContact="editContacts"
  />

  <div v-if="is_modal" :key="render">
    <CollectionAdd @contacts="employeeContacts" />
  </div>
  <div v-if="is_edit" :key="render">
    <CollectionEdit :contacts="editContacts" />
  </div>
  <div v-if="is_delete" :key="render">
    <CollectionDelete @deleteModal="deleteModal" />
  </div>
...
const employeeContacts = (data: any) => {
